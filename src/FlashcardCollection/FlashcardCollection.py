...
class FlashcardCollection:
    ...
    def __init__(self, directory='%USERPROFILE%/.FlashcardCollection'):
        '''
        Represents a collection of flashcard sets

        :param directory: path to flashcard directory; defaults to %USERPROFILE%/.FlashcardCollection; set to '' for testing
    ...
    def populate_flashcards(self, no_of_flashcards: int):
        ...
        if no_of_flashcards == 0:
            ...
            for i in range(no_of_flashcards):
                if not os.path.isfile(f"{self.path_to_flashcards}/flashcardset_{i}"):
